...
class Refined(object):
    ...
    def process_tensors(self, batch: BatchedElementsTns, ner_threshold: float = 0.5,
        ...
        for span_idx, span in enumerate(spans):
            ...
            ###
            # for entity_id, score in zip(sorted_entity_ids[span_idx], sorted_entity_ids_scores[span_idx]):
            ...
            ###
            # sum_score = 0
            # for pred_ent in span.top_k_predicted_entities:
            #     sum_score += pred_ent[1]
            ...
            ###
            mention = []
            for id, score in zip(sorted_entity_ids[span_idx], sorted_entity_ids_scores[span_idx]):
                if id != 0:
                    entity = {}
                    entity["wikidata_qid"] = f"Q{id}"
                    if self.preprocessor.qcode_to_wiki is not None: 
                        entity["title"] = self.preprocessor.qcode_to_wiki.get(wikidata_id)
                    else:
                        entity["title"] = None
                    entity["score"] = score
                    mention.append(entity)
            top_k_predicted_entities_2.append(mention)
            # print("mention")
            # print(mention)
            # print("----------" * 10)
            ###
            ...
            ###
            # sum_conf = 0
            # for cand_ent in span.candidate_entities:
            #     sum_conf += cand_ent[1]
            # print(f"sum_conf = {sum_conf}")
            # print("----------" * 10)
            ###
        ...
        return top_k_predicted_entities_2
